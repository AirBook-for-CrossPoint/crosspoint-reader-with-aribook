...
void BluetoothReceiveActivity::onEnter() {
  Activity::onEnter();
  animationFrame = 0;
...
void BluetoothReceiveActivity::onExit() {
  receiver.stop();
  Activity::onExit();
}

void BluetoothReceiveActivity::loop() {
  ...
  if (now - lastStatusPollAt >= STATUS_POLL_INTERVAL_MS) {
    ...
    if (snapshot.state != lastSnapshot.state || snapshot.connected != lastSnapshot.connected ||
        snapshot.bytesReceived != lastSnapshot.bytesReceived || snapshot.bytesExpected != lastSnapshot.bytesExpected ||
        snapshot.fileName != lastSnapshot.fileName || snapshot.lastCompleteName != lastSnapshot.lastCompleteName ||
...
void BluetoothReceiveActivity::render(RenderLock&&) {
  renderer.clearScreen();

  const auto& metrics = UITheme::getInstance().getMetrics();
  const int pageWidth = renderer.getScreenWidth();

  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_BLUETOOTH_RECEIVE),
                 tr(STR_CROSSPOINT_AIRBOOK));
...
void BluetoothReceiveActivity::renderStatus(const BluetoothFileReceiver::StatusSnapshot& status) const {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int pageWidth = renderer.getScreenWidth();
  ...
  switch (status.state) {
    ...
    case BluetoothFileReceiver::State::Waiting:
      ...
      detail = tr(STR_OPEN_AIRBOOK_APP);
      break;
    case BluetoothFileReceiver::State::Connected:
      title = tr(STR_AIRBOOK_CONNECTED);
      detail = tr(STR_SEND_BOOK_FROM_APP);
      break;
    ...
    case BluetoothFileReceiver::State::Complete:
      title = tr(STR_BLUETOOTH_RECEIVED);
      detail = status.lastCompleteName.empty() ? status.fileName : status.lastCompleteName;
      break;
...
void BluetoothReceiveActivity::drawBluetoothMark(const int centerX, const int centerY, const uint8_t frame) const {
...
void BluetoothReceiveActivity::drawProgress(const int x, const int y, const int width, const int progress) const {
